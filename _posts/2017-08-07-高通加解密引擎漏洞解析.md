...
### CVE-2016-6738 漏洞补丁
<span id="CVE-2016-6738-patch"></span> [top](#top)
...
如上，**qcedev_check_sha_params** 对传入的数据数组挨个累加长度，并对总长度做整数溢出校验，问题在于， ‘req->data[i].len’ 是 uint32_t 类型， 总长度 ‘total’ 也是 uint32_t 类型，uint32_t 的上限是 UINT_MAX, 而这里使用了 ULONG_MAX 来做校验
...
所以这里的整数溢出校验在64bit系统是无效的，即在64bit系统，req->data 数组项的总长度可以整数溢出，这是得到一个疑似漏洞。

下面看看这里的整数溢出有何影响，我们选取 case QCEDEV_IOCTL_SHA_UPDATE_REQ
...
在函数 qcedev_hash_cmac 里， line 900 申请的堆内存 k_buf_src 的长度是 qcedev_areq->sha_op_req.data_len ，即请求数组里所有项的长度之和， 然后在 line 911 ~ 920 的循环里，会将请求数组 qcedev_areq->sha_op_req.data[] 里的元素挨个拷贝到 k_buf_src 里，由于前面存在的整数溢出漏洞，这里会转变成为一个堆溢出漏洞，至此漏洞坐实。

### CVE-2016-3935 漏洞补丁
<span id="CVE-2016-3935-patch"></span> [top](#top)

![](imgs-20170807/3935patch.png)

这个 [漏洞补丁](https://source.codeaurora.org/quic/la/kernel/msm-3.18/commit/?id=5f69ccf3b011c1d14a1b1b00dbaacf74307c9132) 也很直观，就是在做整数溢出时，将 ULONG_MAX 改成了 U32_MAX
...
#### 什么是提权
<span id="what-is-kernel-exp"></span> [top](#top)
...
linux 系统里，进程由 struct task_struct 表示，进程的权限由该结构体的两个成员 real_cred 和 cred 表示
...
所谓提权，就是修改进程的 real_cred/cred 这两个结构体的值，随着缓解措施的不断演进，完整的提权过程还需要修改其他一些内核变量的值，但是最基础的提权还是修改本进程的 cred。 这个任务又可以分解为多个问题：
...
#### 利用方法回顾
<span id="android-kernel-exp"></span> [top](#top)

![](imgs-20170807/exphistory.png)

[图片来自 ： http://powerofcommunity.net/poc2016/x82.pdf]

上图是最近若干年围绕 android kernel 漏洞利用和缓解的简单回顾，在2009的时候，由于没有对 mmap 的地址范围做任何限制，应用层可以映射0页面，null pointer deref 漏洞在当时也是可以做利用的，后面针对这种漏洞推出了 mmap_min_addr 限制，目前所有 null pointer deref 漏洞只能造成 dos.  早期，常用的提权套路是从 /proc/kallsyms 搜索符号 commit_creds 和 prepare_kernel_cred 的地址，利用漏洞控制内核流程执行 commit_creds(prepare_kernel_cred(0)) 就可以完成提权，针对这一种方法，内核推出了 
kptr_restrict/dmesg_restrict ，使得默认配置下无法搜索任何内核符号的地址。2013 推出 text RO 和 PXN 等措施，通过漏洞改写内核代码实现提权或者直接跳转到用户态执行shellcode的提权方式也失效了。在PXN时代，主要的提权思路是使用rop，但是rop的特点适用于执行比较简单的操作，所以有一种思路是使用rop改写 addr_limit 的值，破除本进程的系统调用 access_ok 校验，然后通过一些函数实现直接内核读写来提权，还有就是本文使用的思路，用漏洞重定向内核驱动的 file_operations 结构体到应用层，再用rop地址填充应用层的伪 file_operations 里的函数指针。
...
针对这些提权方法，该项目开发了对应的一系列缓解措施，目前这些措施正在逐步推入linux kernel 主线，下面是其中一部分缓解方案，可以看到，我们回顾的所有利用方法都已经被考虑在内，不久的将来，这些方法可能都会失效，linux kernel 利用之路必将越走越艰辛。
...
#### 本文使用的利用方法
<span id="this-kernel-exp"></span> [top](#top)
...
<span id="CVE-2016-6738-exp"></span> [top](#top)
...
<span id="CVE-2016-3935-exp"></span> [top](#top)
